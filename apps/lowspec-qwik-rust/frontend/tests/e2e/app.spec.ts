...
async function expectGraphqlOk<T = Record<string, unknown>>(res: Response): Promise<T> {
...
async function waitForAppTableReady(page: Page) {
  await expect(
...
async function narrowItemsByFilter(page: Page, q: string) {
