...
/** Vite プロキシ先。`BACKEND_URL` が優先、なければ `BACKEND_HOST` + `BACKEND_PORT`（backend の `BIND_ADDR` と揃える）。 */
function resolveBackendOrigin(env: Record<string, string>): string {
  ...
  const port = env.BACKEND_PORT?.trim() || "8080";
  ...
  return {
    ...
    server: {
      ...
      headers: {
        ...
        // 改修案 P3b: DuckDB-WASM / 共有バッファ向け COI（本番も同様を推奨）
